...
const debug = require('debug')('elasticsearch-restaurants-filter-api-nodejs:helper');

var key_aggregations = [
  {
    "name": "published_on",
...
var get_filter_aggregations = function () {
  var z = {};
...
var convert_sort = function (sorts) {
...
var add_search_text = function (queries, searchTextOptions) {
...
var convert_to_es_search_params = function (params) {
  var main = {};
  ...
  key_aggregations.map(item => {
    item.aggs.map(x => {
      ...
      if (type) {
        ...
        var existed = false;
        main[type].find(q => {
          var keys_1 = Object.keys(q)[0];
          var keys_q = Object.keys(query)[0];
          if (keys_1 == keys_q) {
            var keys_2 = Object.keys(q[keys_1])[0];
            var keys_2q = Object.keys(query[keys_1])[0];
            if (keys_2 === keys_2q) {
              existed = true;
              q[keys_1][keys_2] = [...new Set(q[keys_1][keys_2].concat(query[keys_1][keys_2]))];
            }
          }
        });
  ...
  if (Array.isArray(main['filter']) && main['filter'].length > 0)
    main['filter'] = contains_values_filters(main['filter']);
...
var get_filters_for_ui = function (aggregations) {
  var z = [];
  ...
  key_aggregations.map(item => {
    item.aggs.map(x => {
      var found = z.find(a => a.title == x.title);
      if (!found) {
        found = { name: x.key, title: x.title, options: [], type: item.type };
        z.push(found);
      }
      var { doc_count, buckets } = aggregations[x.key];
      if (Array.isArray(buckets)) {
        found.options = buckets.map(b => {
          var text = b.key;
          if (b.key_as_string)
            text = b.key === 0 ? 'No' : 'Yes';
          var value = b.key.toString();
          if (item.type === "date-range-filter")
